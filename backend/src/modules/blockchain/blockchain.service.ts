...
export class BlockchainService implements OnModuleInit {
  ...
  async getBalance(cryptoIndex: number): Promise<{ balance: number }> {
    ...
    try {
      ...
      return { balance: Number(ethers.formatEther(balance)) };
    } catch (error) {
  ...
  async sendEther({
    ...
    to,
    amountEther,
  }: SendEtherDto): Promise<{ receipt_hash: string; msg: string }> {
    ...
    return { receipt_hash: receipt?.hash ?? '', msg: 'OK' };
  }
